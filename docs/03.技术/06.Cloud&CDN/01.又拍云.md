...
2. 云储存针对文件加速，CNAME配置完成后即可，通过路径访问图片（图片可直接上传）。
具体内容参考官网，主
...
- GitHub：这个就不多说了，参考 [官方文档](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token) 生成一个 **token** 密匙一会要用。
...
- 首先通过通过 **TinyPNG** 把图片做最优的压缩，然后通过 **PicGo** 批量上传生成图床，最后通过 **jsDelivr** 对 **GitHub** 的加速解决GitHub的访问速度问题。巴适~

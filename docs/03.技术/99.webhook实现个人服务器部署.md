...
核心代码就是 `runCommand` 函数，满足条件执行。这里没有像 `github-webhook-handler` 包一样对密匙进行处理验证，只是简单的验证了身份和条件。

接下来编写我们的bash脚本，这里之后可以优化成执行对应项目下的对应脚本。
...
反复尝试了很多方法，摸索找思路，再不断的改进，最终也是实现了当初的想法，不过也确实走了不少弯路。在此，把整个过程记录下来，供自己参考也为了加深理解，希望能帮到有需要的小伙伴，少走些弯路~

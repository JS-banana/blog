...
+ # deploy to coding
+ codingUrl=https://ptzv1yuleer1:${CODING_TOKEN}@e.coding.net/ssscode/blog/vuepress.git #注意！！！这里需要使用coding提供的个人令牌的用户名和token （https://[name]:[token]@e.coding.net/xx）
...
反复尝试了很多方法，一点点找思路，再不断的改进，最终也是实现了当初的想法，不过也确实走了不少弯路。在此，把整个过程记录下来，供自己参考也为了加深理解，希望能帮到有需要的小伙伴，少走些弯路~

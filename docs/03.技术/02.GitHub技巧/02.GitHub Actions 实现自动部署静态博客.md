...
一开始的部署方式是使用[sh部署脚本](https://github.com/JS-banana/vuepress-theme-vdoing/blob/master/deploy.sh)把代码提交到这两个平台的仓库分支，虽然已经很方便了，但是我还想把博客未打包的源码提交到Github主分支上。这就需要我操作两次命令，**我就想能不能只需要一次操作就可以同时把源码、部署代码一次性提交到两个平台呢？**
...
![token设置](https://cdn.jsdelivr.net/gh/sunss/image_store/blog/20200103124812.jpg 'token设置')

GitHub Actions 的配置文件叫做 workflow 文件，存放在代码仓库的`.github/workflows`目录。

workflow 文件采用 [YAML 格式](https://sunss.com/pages/4e8444e2d534d14f/)，文件名可以任意取，但是后缀名统一为`.yml`，比如`ci.yml`。一个库可以有多个 workflow 文件。GitHub 只要发现`.github/workflows`目录里面有`.yml`文件，就会自动运行该文件。    
...
# deploy to github
echo 'blog.sunss.com' > CNAME
if [ -z "$GITHUB_TOKEN" ]; then
  msg='deploy'
  githubUrl=git@github.com:sunss/blog.git
else
  msg='来自github action的自动部署'
  githubUrl=https://sunss:${GITHUB_TOKEN}@github.com/sunss/blog.git
...
if [ -z "$CODING_TOKEN" ]; then  # -z 字符串 长度为0则为true；$CODING_TOKEN来自于github仓库`Settings/Secrets`设置的私密环境变量
  codingUrl=git@git.dev.tencent.com:sunss/sunss.git
else
  codingUrl=https://sunss:${CODING_TOKEN}@git.dev.tencent.com/sunss/sunss.git
fi
git add -A
git commit -m "${msg}"
...
![部署日志](https://cdn.jsdelivr.net/gh/sunss/image_store/blog/20200103124813.png '部署日志')



## 相关文章

[《GitHub Actions 定时运行代码：每天定时百度链接推送》](https://sunss.com/pages/f44d2f9ad04ab8d3/)

...
`Vue`在初始化时会有个依赖收集处理，通过对属性和指令的遍历处理（这里的属性包括 `props`、`data`、`computed` 等，指令是在 `render` 的时候通过模板编译进行过滤处理得到），得到需要进行响应式处理的属性，然后再通过 `Observer`、`Dep`、`Watcher` 这几个类进行订阅与监听。

其中通过 `Object.defineProperty()` 这个方法对数据劫持，并重新定义 `set` 和 `get` 方法，一旦数据变动，收到通知，更新视图。

    ...
    <MainLayout>
      ...
      <template #mainRight>
        ...
        <div class="categories-wrapper card-box">
          <div class="title">
            <img
              style="width: 18px; vertical-align: -2px"
              src="https://cdn.jsdelivr.net/gh/JS-banana/images/vuepress/youqinglianjie.png"
              alt=""
            />
            传送门
          </div>
          <div class="categories">
            <a
              :key="index"
              v-for="(item, index) in $themeConfig.friends"
              :href="item.link"
              target="_blank"
            >
              {{ item.name }}
    ...
    bannerBgStyle() {
      ...
      if (!bannerBg || bannerBg === 'auto') {
        ...
        if (this.$themeConfig.bodyBgImg) {
          ...
          // 网格纹背景
          if (this.$themeConfig.bannerImg) return `background:url(${this.$themeConfig.bannerImg})`;
